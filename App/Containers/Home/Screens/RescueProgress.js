...
class RescueProgress extends PureComponent {
    constructor() {
        super()
        this.state = {
...
class MessageListView extends React.Component {
    constructor(props) {
        super(props)
        this.state = { messageDialog: false }
    }
    render() {
        const { messages, navigate, users } = this.props
        ...
        return (
            <View style={styles.container} >
                <ScrollView style={styles.container} >
                    {
                        messages && messages.map((message, index) => {
                            let otherId = message.user1 === Meteor.userId() ? message.user2 : message.user1
                            let userArray = users.filter(user => user._id === otherId)
                            if (userArray.length <= 0) return null
                            let otherUser = userArray[0]
                            return (
                                <View style={{ backgroundColor: "#fff", flex: 1 }}>
                                    <View style={styles.contactItems}>
                                        <Image source={Images.adrianImg} style={{ alignItems: 'center' }} ></Image>
                                        <View style={styles.messageContact} >
                                            <Text style={styles.NormalTextBlack}>{otherUser.profile.name}</Text>
                                            <Text style={styles.smallTextBlack}>{message.chats[message.chats.length - 1]}</Text>
                                        </View>
                                        <Text style={{ color: '#7745A9' }}>{moment(message.lastUpdated).toNow()}</Text>
                                    </View>
                                </View>
                            )
                ...
                <DescriptionModal descriptionModalVisible={this.state.messageDialog} close={this.closeDescriptionModal} />
                <TouchableOpacity onPress = {() => this.setState({messageDialog : true})}>
                    <Image source={Images.btnContact} style={styles.contactBtn} />
                </TouchableOpacity>
            </View>
        )
    }
...
const MessageListViewContainer = createContainer((params) => {
    Meteor.subscribe('getmessages', this.props.donationId)
    return ({
        messages: Meteor.collection('Messages').find({ $or: [{ user1: Meteor.userId() }, { user2: Meteor.userId() }] }),
        reservation: Meteor.collection('Reservation').find({donationId: donationId}),
        users: Meteor.collection('users').find({})
    })
}, MessageListView)


export default SideHOC(RescueProgress)